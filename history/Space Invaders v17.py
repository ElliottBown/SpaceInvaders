...
GRAPHIC_GUN_EXPLODING = 'Graphics\Gun exploding.png'
...
# Delays
BULLET_DELAY=0.5 # Seconds
ANIMATION_DELAY=0.5 # Alien # Seconds
SPACESHIP_DELAY=25 # Seconds
GUN_EXPLODE_DELAY=2 # Seconds
...
# Other
MISSILE_CHANCE_PER_SECOND=3/1000
...
class Gun(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.imageNormal=pygame.image.load(GRAPHIC_GUN)
        self.imageExploding=pygame.image.load(GRAPHIC_GUN_EXPLODING)
    ...
    def explode(self):
        self.image = self.imageExploding
        self.exploding = True

    def stop_explode(self):
        self.image = self.imageNormal
        self.exploding = False
...
class Alien(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.alien_above=None
        self.imageRed=pygame.image.load('Graphics\Alien Red.png') # temporary - can be deleted later
    ...
    def update (self):
        if self.alien_below is None:
            ...
            if random.random()< MISSILE_CHANCE_PER_SECOND:
                ...
                Alien.all_missiles.add(missile)
...
class Alien_swarm():
    ...
    def check_for_collisions(self,bullet):
        ...
        for hit in hits:
            if hit.alien_above is not None:
                hit.alien_above.alien_below=hit.alien_below
            if hit.alien_below is not None:
                hit.alien_below.alien_above=hit.alien_above
            hit.kill()
            bullet.kill()
...
class Game(object):
    ...
    last_gun_explode_time=time.time()
    ...
    def __init__(self):
        ...
        Alien.all_missiles = self.all_missiles
        ...
        for col in range(1,ALIEN_COLUMNS+1):
            ...
            for row in range(5,0,-1):
                ...
                if previous_alien is not None:
                    ...
                    previous_alien.alien_above=alien
    ...
    def run_logic(self):
        ...
        if self.game_state==GameState.GAME_RUNNING:
            ...
            if time.time()>self.last_spaceship_time+SPACESHIP_DELAY:
                ...
                print('2) direction should have chaged, self.next_spaceship_direction=',self.next_spaceship_direction)
            ...
            # Check for bullet colissions with missiles
            hits=pygame.sprite.spritecollide(self.bullet,self.all_missiles,False)
            for hit in hits:
                hit.kill()
                self.bullet.kill()

            # Check for any missile colliding with the Gun
            if not self.gun.exploding:
                hits=pygame.sprite.spritecollide(self.gun,self.all_missiles,False)
                for hit in hits:
                    hit.kill()
                    self.gun.explode()
                    self.lives.lose_life()
                    self.last_gun_explode_time=time.time()
                
            # Check if it is time to create a new gun after it explodes
            if self.gun.exploding:
                if time.time() > self.last_gun_explode_time + GUN_EXPLODE_DELAY:
                    self.gun.stop_explode()
